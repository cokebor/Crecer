...
namespace Utilidades
{
    ...
    {
        public static string CodificarPasword(string usuario, string password)
        {
            string res;
            res = string.Concat(usuario, password);
            SHA1 sha1 = new SHA1CryptoServiceProvider();
            byte[] inputBytes = (new UnicodeEncoding()).GetBytes(res);
            byte[] hash = sha1.ComputeHash(inputBytes);

            return Convert.ToBase64String(hash);
        }

        private static string llave = "A12B34C56DAA1122BB3344CC5566DD";
